 ...
 * env.CLAUDECHIP_API_KEY, env.ANTHROPIC_AUTH_TOKEN). Those settings exist for
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // ~/.claude settings (apiKeyHelper, settings.env.CLAUDECHIP_API_KEY) MUST NOT
  ...
  const hasExternalApiKey =
    apiKeySource === 'CLAUDECHIP_API_KEY' || apiKeySource === 'apiKeyHelper'
  ...
  // 3. User has an external API key via apiKeyHelper (not CLAUDECHIP_API_KEY)
  // Note: CLAUDECHIP_API_KEY is allowed for first-party auth to enable env-based API key usage
  ...
  const shouldDisableAuth =
    ...
    (apiKeySource === 'apiKeyHelper' && !isManagedOAuthContext())
...
export function getAnthropicApiKeyWithSource(
  ...
  // --bare: hermetic auth. Only CLAUDECHIP_API_KEY env or apiKeyHelper from
  ...
  // Also supports CLAUDECHIP_API_KEY as a fallback for custom deployments.
  if (isBareMode()) {
    if (process.env.CLAUDECHIP_API_KEY) {
      return { key: process.env.CLAUDECHIP_API_KEY, source: 'CLAUDECHIP_API_KEY' }
    }
    if (process.env.CLAUDECHIP_API_KEY) {
      return { key: process.env.CLAUDECHIP_API_KEY, source: 'CLAUDECHIP_API_KEY' }
    }
  ...
  // On homespace, don't use CLAUDECHIP_API_KEY (use Console key instead)
  // https://anthropic.slack.com/archives/C08428WSLKV/p1747331773214779
  // Also supports CLAUDECHIP_API_KEY as a fallback for custom deployments.
  const apiKeyEnv = isRunningOnHomespace()
    ? undefined
    : (process.env.CLAUDECHIP_API_KEY || process.env.CLAUDECHIP_API_KEY)
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    if (
      ...
      throw new Error(
        'CLAUDECHIP_API_KEY or CLAUDE_CODE_OAUTH_TOKEN env var is required',
      )
    }
  ...
  // Check for CLAUDECHIP_API_KEY before checking the apiKeyHelper or /login-managed key
  if (
    apiKeyEnv &&
