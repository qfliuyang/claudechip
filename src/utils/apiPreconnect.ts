 ...
 * so CLAUDECHIP_BASE_URL/proxy/mTLS in settings would be invisible and preconnect
...
export function preconnectAnthropicApi(): void {
  ...
  // CLAUDECHIP_BASE_URL env + USE_STAGING_OAUTH + USE_LOCAL_OAUTH in one lookup.
  // NODE_EXTRA_CA_CERTS no longer a skip — init.ts applied it before this fires.
  const baseUrl =
    process.env.CLAUDECHIP_BASE_URL || getOauthConfig().BASE_API_URL
