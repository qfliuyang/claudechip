...
function withoutSSHTunnelVars(
  ...
  const {
    ...
    CLAUDECHIP_BASE_URL: _2,
    CLAUDECHIP_API_KEY: _3,
 ...
 * traffic (e.g., CLAUDECHIP_BASE_URL) to an attacker-controlled server.
 */
 ...
 * CLAUDECHIP_BASE_URL take effect during first-run/onboarding.
 *
 * For trusted sources (user settings, managed settings, CLI flags), ALL env vars
 * are applied — including ones like CLAUDECHIP_BASE_URL that would be dangerous
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // CLAUDECHIP_BASE_URL — both settable via settings.env.
