...
export async function launchRepl(root: Root, appProps: AppWrapperProps, replProps: REPLProps, renderAndRun: (root: Root, element: React.ReactNode) => Promise<void>): Promise<void> {
  process.stdout.write('[REPL] launchRepl start\n');
  const {
    App
  } = await import('./components/App.js');
  process.stdout.write('[REPL] App imported\n');
  const {
    REPL
  } = await import('./screens/REPL.js');
  process.stdout.write('[REPL] REPL imported\n');
  process.stdout.write('[REPL] about to call renderAndRun\n');
  await renderAndRun(root, <App {...appProps}>
      <REPL {...replProps} />
    </App>);
  process.stdout.write('[REPL] renderAndRun done\n');
}
